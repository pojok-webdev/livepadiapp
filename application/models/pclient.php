...
class Pclient extends CI_Model{
    ...
    function populateclient($status=array('9'),$active=array('1'),$user=null){
        ...
        if(is_null($user)){
            ...
            $sql.= 'a.sale_id,';
            $sql.= "group_concat(distinct i.name)fbpic, ";
            $sql.= "case clientcategory ";
            $sql.= "when '2' then 'Platinum' ";
            $sql.= "when '3' then 'Gold' ";
            $sql.= "when '4' then 'Silver' ";
            ...
            $sql.= "left outer join (select nofb,client_id from fbs where status='1') h on h.client_id=a.id ";
            $sql.= "left outer join (select nofb,client_id,concat(name,'(',hp,')')name from fbpics where role='teknis') i on i.client_id=a.id and i.nofb=h.nofb ";
            ...
            $sql = "select a.id,a.name,a.address,d.username usrname,a.alias,a.phone_area,a.phone,e.pic,";
            $sql.= "group_concat(g.name)ap, ";
            $sql.= "group_concat(distinct i.name)fbpic, ";
            $sql.= 'a.sale_id,';
            $sql.= "case clientcategory ";
            $sql.= "when '2' then 'Platinum' ";
            ...
            $sql.= "left outer join aps g on g.id=f.ap_id ";
            $sql.= "left outer join (select nofb,client_id from fbs where status='1') h on h.client_id=a.id ";
            $sql.= "left outer join (select nofb,client_id,concat(name,'(',hp,')')name from fbpics where role='teknis') i on i.client_id=a.id and h.nofb=i.nofb";
    ...
    function changefarmer($params){
        $sql = 'update clients set sale_id= '.$params['sale_id'].' where id=' .$params['id'];
        $ci = & get_instance();
        $ci->db->query($sql);
        return $sql;
    }
    ...
    function get_sites($client_id){
        $sql = 'select id,address,pic_name,pic_phone,pic_phone_area from client_sites where client_id='.$client_id.' ';
        $ci = & get_instance();
		$que = $ci->db->query($sql);
        return array(
            'res'=>$que->result(),
    ...
    function getclientexcel(){
        $sql = "select a.id,a.name,a.alias,a.address,";
        $sql.= 'case a.isffr when "0" then "Non FFR" when "1" then "FFR" end isffr, ';
        $sql.= 'joindate,';
        $sql.= 'case branch_id ';
        ...
        $sql.= "when '3' then 'Gold' ";
        $sql.= "when '4' then 'Silver' ";
        $sql.= "when '5' then 'Bronze' else 'uncategorized' end clientcategory,";
        $sql.= "b.id hunter_id,b.username hunter,c.id farmer_id,c.username farmer, group_concat(e.sla) sla from clients a ";
        $sql.= "left outer join users b on b.id=a.user_id ";
        $sql.= "left outer join users c on c.id=a.sale_id ";
        $sql.= 'left outer join fbs d on d.client_id=a.id ';
        $sql.= 'left outer join fbsslas e on e.nofb=d.nofb ';
        $sql.= "where a.status in ('1','9') and a.active='1' and a.hide='0' " ;
        $sql.= "group by a.id,a.name,a.isffr,a.alias,a.address,a.branch_id,a.clientcategory,b.id,b.username,c.id,c.username ";
        $sql.= 'order by a.branch_id asc,a.id asc ';
        $ci = & get_instance();
		$que = $ci->db->query($sql);
        return array(
            'res'=>$que->result(),
    ...
    function getsalescombodata(){
        $sql = 'select distinct b.id,b.username ';
        $sql.= 'from roles_users a ';
        $sql.= 'left outer join users b on b.id=a.user_id ';
        $sql.= 'left outer join roles c on c.id=a.role_id ';
        $sql.= 'where a.role_id in(6,4,5,3) and b.active="1"';
        $ci = & get_instance();
		$que = $ci->db->query($sql);
        return array(
            'res'=>$que->result(),
    ...
    function saveptp(){
        ...
        $sql.= 'values ';
        $sql.= '('.$params['client_id'].','.$params['address'].') ';
        $que = $ci->db->query($sql);
        return array(
            'id'=>$ci->db->insert_id()
        );
