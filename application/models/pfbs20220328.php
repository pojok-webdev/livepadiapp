...
class Pfbs extends CI_Model{
	...
	function mergefb($previd,$nextid){
		$sql = 'insert into movedfb select *,'.$nextid.' destination,current_timestamp() movedate from fbs ';
		$sql.= 'where client_id='.$previd.' ';
		$ci = & get_instance();
		$que = $ci->db->query($sql);

		$sql = 'update fbs set client_id='.$nextid.' ';
		$sql.= 'where client_id ='. $previd. ' ';
		$ci = & get_instance();
		$que = $ci->db->query($sql);

		$update = 'update clients set active="0" where id='.$previd.' ';
		$ci = & get_instance();
		$que = $ci->db->query($update);

		$log = 'insert into app_logs (user,subject,description,ipaddr) ';
		$log.= 'values ';
		$log.= '("'.$ci->session->userdata['username'].'","movefb","move fb from '.$previd.' to '.$nextid.'","'. $_SERVER["REMOTE_ADDR"].'") ';
		$ci = & get_instance();
		$que = $ci->db->query($log);
    ...
    function getdocumentarray(){
        return array("npwp","siup","sppkp","ktp penanggung jawab","ktp pemohon","surat kuasa");
    }
    function getpc(){
	...
	function getsohos(){
		return array(1=>"Padi SOHO 1:4",2=>"Padi SOHO 1:2");
	}
...
//		$sql.= "b.description,";
	...
	function getdevicesbynofb($nofb){
		$sql = "select a.id,a.device_id,b.price,b.name from fbdevices a left outer join pricelists2.devices b on b.id=a.device_id ";
        $sql.= "where a.fb_id='".$nofb."' ";
		$ci = & get_instance();
        $que = $ci->db->query($sql);
        $amount = $que->num_rows();
        $res = $que->result();
        return array('amount'=>$amount,'res'=>$res);
	}
	function getfbvas($nofb){
		$sql = "select a.id,a.vas_id,a.price,b.name from fbvases a left outer join pricelists2.vases b on b.id=a.vas_id ";
        $sql.= "where a.fb_id='".$nofb."' ";
		$ci = & get_instance();
        $que = $ci->db->query($sql);
        $amount = $que->num_rows();
...
return array('sql'=>$sql);
    ...
    function savefbvas($params){
        $ci = & get_instance();
        $sql = "";
		$sql = "insert into fbvases ";
		$sql.= "(fb_id,vas_id,createuser) ";
		$sql.= "values ";
		$sql.= "('".$params['fb_id']."','".$params['vas_id']."','".$params['createuser']."') ";
		$que = $ci->db->query($sql);
		return array('lastid'=>$ci->db->insert_id(),'sql'=>$sql);
    }
