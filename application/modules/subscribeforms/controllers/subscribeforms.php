...
class Subscribeforms extends CI_Controller{
	...
	function getclients(){
		...
		foreach($objs['res'] as $obj){
			$out[$obj->id] = $obj->id . ' - ' . $obj->name;
		}
		return $out;
	}
	function mergefb(){
		$previd = $this->uri->segment(3);
		$nextid = $this->uri->segment(4);
		echo json_encode($this->pfbs->mergefb($previd,$nextid));
	}
	function index(){
		padi_checklogin();
		$objs=getclients();
		...
		$this->load->view('Sales/subscribeforms/index20211004',$data);
	}
	function edit(){
		padi_checklogin();
	...
	function setfbcomplete(){
		$params = $this->input->post();
		$obj = new Client_site();
		$obj->where('id',$params['client_id'])->update(array('fbcomplete'=>'1'));
		echo $params['id'];
	}
	function getpics(){
		$this->load->model('fb_fee');
		...
		foreach($fbpic as $pic){
			switch(strtoupper($pic->role)){
				...
				break;
				case 'TEKNIS':
					$tek = '"id":"'.$pic->client_id.'","name":"'.$pic->name.'","position":"'.$pic->position.'","telp_hp":"'.$pic->telp_hp .'","phone":"'.$pic->phone.'","hp":"'.$pic->hp.'","email":"'.$pic->email.'","idnum":"'.$pic->idnum.'"';
				break;
				case 'RESP':
				...
				break;
				case 'BILLING':
					$bill = '"id":"'.$pic->client_id.'","name":"'.$pic->name.'","position":"'.$pic->position.'","telp_hp":"'.$pic->telp_hp .'","phone":"'.$pic->phone.'","hp":"'.$pic->hp.'","email":"'.$pic->email.'","idnum":"'.$pic->idnum.'"';
				break;
				case 'SUPPORT':
					$support = '"id":"'.$pic->client_id.'","name":"'.$pic->name.'","telp_hp":"'.$pic->telp_hp .'","phone":"'.$pic->phone.'","hp":"'.$pic->hp.'","email":"'.$pic->email.'","idnum":"'.$pic->idnum.'"';
				break;
				case 'ADM':
				$adm = '"id":"'.$pic->client_id.'","name":"'.$pic->name.'","position":"'.$pic->position.'","telp_hp":"'.$pic->telp_hp .'","phone":"'.$pic->phone.'","hp":"'.$pic->hp.'","email":"'.$pic->email.'","idnum":"'.$pic->idnum.'"';
				break;
			}
		}
		$str = '{'.$nofb.',';
