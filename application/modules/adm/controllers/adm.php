...
function saveImage($directory,$img,$imgname){
	$img = str_replace('data:image/jpeg;base64,', '', $img);
	$img = str_replace(' ', '+', $img);
	$fileData = base64_decode($img);
	$fileName = $this->config->item('appimages').$directory.'/'.$imgname.'.jpg';

	return file_put_contents($fileName, $fileData);
}
function save_image(){
	$params = $this->input->post();
	json_encode($this->saveImage('surveys',$params['image'],$params['imagename']));
	//return '{result:"OK"}';
	echo json_encode($params);
}
function save_installimage(){
	$params = $this->input->post();
	json_encode($this->saveImage('installs',$params['image'],$params['imagename']));
	//return '{result:"OK"}';
	echo json_encode($params);
}
}
