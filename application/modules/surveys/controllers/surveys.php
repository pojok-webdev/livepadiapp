...
class Surveys extends CI_Controller {
	...
	function edit() {
		...
		$id = $this->uri->segment(3);//survey_site_id
		$keyvalpaired = false;
		...
		$this->load->model('pservice');$this->load->library('lib_array');$internets = $this->pservice->getinternets();$vases = $this->pservice->getvases();$philoins = $this->pservice->getphiloins();$devices = $this->pservice->getdevices();
		...
		$this->data['internets']=$pinternets;$this->data['vases']=$pvases;$this->data['devices']=$pdevices;$this->data['philoins']=$pphiloins;
		switch ($this->session->userdata["role"]) {
			...
			case "Sales":
				if($this->common->is_decessor($obj->client_site->client->sale_id,$myuser->id)||($obj->client_site->client->sale_id===$myuser->id)){
						$this->load->view('Sales/surveys/edit20210923', $this->data);
				}else{
