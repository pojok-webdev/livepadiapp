...
class Survey_requests extends CI_Controller {
	...
	function add() {
		...
		$this->data['menuFeed'] = 'survey';$this->data['internets']=$pinternets;$this->data['vases']=$pvases;$this->data['devices']=$pdevices;$this->data['philoins']=$pphiloins;
		$this->load->view('Sales/surveys/add20210922', $this->data);
	}
	function addptp(){
		$params = $this->input->post();
		echo json_encode($this->pclient_site->save2(array('client_id'=>$params['client_id'],'address'=>$params['ptpaddress'])));
	}
	function addsurveysite(){
		$params = $this->input->post();
		echo json_encode($this->psurvey_site->save(array('client_id'=>$params['client_id'],'address'=>$params['ptpaddress'])));
	}
	function addservice(){
		$params = $this->input->post();
		$this->pservice->savesurveyservice($params['client_id'],$params['servicetype'],$params['product_id']);
		echo json_encode($params);
	}
	function getservicesbytype(){
		$params = $this->input->post();
		$objs = $this->pservice->getservicesbytype($params['client_id'],$params['servicetype']);
		echo json_encode($objs['res']);
	}
	function removeservice(){
		$params = $this->input->post();
		$output = $this->pservice->removeservice($params['client_id'],$params['servicetype'],$params['service_id']);
		echo json_encode('{"sql":'.$output.'}');
	}
