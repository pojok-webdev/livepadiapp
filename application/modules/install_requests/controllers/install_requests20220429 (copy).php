...
class Install_requests extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('pinstall_request');$this->load->model('pservice');
...
/*	function getTrial(){
	...
	function install_edit(){
		$sessdata = array("pending_url"=>"/install_requests/install_edit/".$this->uri->segment(3));$this->load->model('pservice');$this->load->library('lib_array');$internets = $this->pservice->getinternets();$vases = $this->pservice->getvases();$philoins = $this->pservice->getphiloins();$devices = $this->pservice->getdevices();
		$id = $this->uri->segment(3);$pinternets = $this->lib_array->createcomboarray($internets['res']);$pvases = $this->lib_array->createcomboarray($vases['res']);$pdevices = $this->lib_array->createcomboarray($devices['res']);$pphiloins = $this->lib_array->createcomboarray($philoins['res']);
		...
		if(!$client_id){
			...
			$getvases = $pi->getvases($client_id);$this->data['client_id'] = $client_id;
			...
			switch($this->session->userdata["role"]){
				case "TS":
				$this->load->view('TS/installs/edit20210924',$this->data);
				break;
				case "Sales":
					if($this->common->is_decessor($obj->sale_id,$myuser->id)||($obj->sale_id===$myuser->id)){
						$this->load->view('Sales/installs/edit20210923',$this->data);
					}else{
	...
	function preview(){
		$vas = new Vas();
		$this->check_login();$this->load->model('pservice');$this->load->library('lib_array');$internets = $this->pservice->getinternets();$vases = $this->pservice->getvases();$philoins = $this->pservice->getphiloins();$devices = $this->pservice->getdevices();
		$id = $this->uri->segment(3);$pinternets = $this->lib_array->createcomboarray($internets['res']);$pvases = $this->lib_array->createcomboarray($vases['res']);$pdevices = $this->lib_array->createcomboarray($devices['res']);$pphiloins = $this->lib_array->createcomboarray($philoins['res']);
		$pi = new Pinstall_request();
		$client_id = $pi->get_client_by_survey_request($this->uri->segment(3));
		$getvases = $pi->getvases($client_id);
		$this->data['obj']=$this->survey_request->get_obj_by_id($this->uri->segment(3));$this->data['client_id'] = $client_id;
		$this->data['clients']=$this->pclient->get_combo_data();
		$this->data['officers']=$this->user->get_user_by_group('TS');
		$this->data['apwifis']=$this->pdevice->get_combo_data(array(13));
		$this->data['pccards']=$this->pdevice->get_combo_data(array(2));
	...
	function getservicesbytype(){
		$params = $this->input->post();
		$objs = $this->pservice->getinstallservicesbytype($params['client_id'],$params['servicetype']);
		echo json_encode($objs['res']);
	}
	function removeservice(){
		$params = $this->input->post();
		$this->pservice->removeinstallservice($params['client_id'],$params['servicetype'],$params['service_id']);
		echo json_encode($params);
	}
	...
	function showreport(){
		...
		$services = getpricelistservices($client->client_id);
		...
		$data = array(
			...
			'bas'=>$bas['res'],'services'=>$services['res']
		);
