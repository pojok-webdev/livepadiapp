...
/*		$objs = new Client();
	...
	function ttg_padinet() {
		...
		$internets = $this->pservice->getinternets();
		$vases = $this->pservice->getvases();
		$philoins = $this->pservice->getphiloins();
		$devices = $this->pservice->getdevices();
		$pinternets = $this->lib_array->createcomboarray($internets['res']);
		$pvases = $this->lib_array->createcomboarray($vases['res']);
		$pdevices = $this->lib_array->createcomboarray($devices['res']);
		$pphiloins = $this->lib_array->createcomboarray($philoins['res']);
		$data = array(
			'budgets' => $this->budget->get_combo_data(),
			'businesstypes' => $this->business_field->get_combo_data(),
			'hours' => $this->common->gethours(),
			...
			'id' => $this->uri->segment(3),
			'menuFeed' => 'suspect','internets'=>$pinternets,'vases'=>$pvases,'devices'=>$pdevices,'philoins'=>$pphiloins
		);
		$this->load->view('Sales/suspects/ttg_padinet', $data);
	}
	function getservicesbytype(){
		$params = $this->input->post();
		$objs = $this->pservice->getsuspectservicesbytype($params['client_id'],$params['servicetype']);
		echo json_encode($objs['res']);
	}
	function addservice(){
		$params = $this->input->post();
		echo json_encode($this->pservice->savesuspectservice($params['client_id'],$params['servicetype'],$params['product_id']));
	}
	function removeservice(){
		$params = $this->input->post();
		$res = $this->pservice->removesuspectservice($params['client_id'],$params['servicetype'],$params['service_id']);
