	...
	$("#btnConfirmYes").click(function(){
		switch($("#confirmModalLabel").html()){
			...
			case "Konfirmasi penghapusan cabang":
					cabang = $('#mySites tbody tr.selected')
					objcabang = {
						id : cabang.attr('rowid'),
						address : cabang.find('.clientaddress').text(),
						nama:$('#name').val(),client_id:$("#client_id").val()
					}
			$.ajax({
				...
				success:function(){
					...
					$.ajax({
						url:'/applogs/create',
						data:{
							subject:'Hapus Cabang',
							description:'Penghapusan cabang dari menu pelanggan ID:'+objcabang.id + ' ,Alamat:' + objcabang.address + ' ,Nama:' + objcabang.nama + ' ,CLIENT ID:' + objcabang.client_id,
							user:$('#createuser').val()
						},
						type:'post'
					})
					.done(res=>{console.log('Sukese menghapus cabang',res)})
					.fail(err=>{console.log('Err menghapus cabang',err)})
				},
				error:function(err){
		...
		$.ajax({
			...
			success:function(data){
				...
				tr+='				</tr>';
					...
					.done(res=>{
						console.log('save log result',res)
					})
					.fail(err=>{
						console.log('save log err',err)
					})
		...
		$.ajax({
			...
			success:function(){
				...
				$.ajax({
					url:'/applogs/create',
					data:{
						subject:'update client site',
						description:'update site dari pelanggan id:'+myid+' '+$('#siteaddress').val()+' '+$('#city').val(),
						user:''+$('#createuser').val()
					},
					type:'post'
				})
				.done(res=>{
					console.log('update log result',res)
				})
				.fail(err=>{
					console.log('save log err',err)
				})
