...
saveClientProduct = obj => {
    $.ajax({
        url:'/clientproducts/create',
        type:'post',
        data:{
            client_id:0,
    ...
    .done(res=>{
        console.log("Success create clientproduct",res)
    })
    .fail(err=>{
        console.log("Err create clientproduct",err)
    })
}
getClientID = (obj,callback) => {
    $.ajax({
        url:'/clientproducts/getclientidbynofb',
        type:'post',
    ...
    .fail(err=>{
        console.log('Err get ClientID',err)
    })
}
renewClientProduct = obj => {
    $.ajax({
        url:'/clientproducts/duplicatetoclientproduct',
        data:obj,
        type:'post',
        dataType:'json'
    })
    .done(res=>{
        console.log('renewClientProduct success',res)
    })
    .fail(err=>{
        console.log('renew Client Product failed',err)
    })
}
saveProduct = (obj,callback) =>{
    console.log('SaveProduct OBJ',obj)
    $.ajax({
    ...
    .done(res=>{
        ...
        getClientID(obj,params=>{
            renewClientProduct(params)
            /*saveClientProduct({
                client_id:client_id,
                category_id:obj.category_id,product_id:obj.product_id,detail:obj.detail,detail_id:obj.detail_id
            })*/
        })
        callback()
    })
