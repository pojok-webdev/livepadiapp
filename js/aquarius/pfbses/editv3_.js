...
callback({lastid:res,device:$('#devices :selected').text()});
	})
	.fail(err=>{
...
saveFBVas = callback=>{
	$.ajax({
		url:'/pfbses/savefbvas',
		data:{
			vas_id:$('#vases').val(),fb_id:$('#no_fb').text(),createuser:$('#createuser').val()
		},
		type:'post',
		dataType:'json'
	})
	.done(res=>{
		console.log('Success save vas',res);
		callback({lastid:res.lastid,vas:$('#vases :selected').text()})
	})
	.fail(err=>{
		console.log('Error save vas',err);
	});
}
...
console.log('Device',device)
				tr='<tr thisid='+device.lastid+'>';
				tr+='<td class="servicecategory">Perangkat</td>';
				tr+='<td class="info">';
				tr+='<a>'+device.device+'</a>';
				tr+='</td>';
				tr+='<td>';
				...
				tr+='<li class="remove_device"><a>Hapus</a></li>';
				tr+='</ul>';
				tr+='</div>';
				tr+='	</td>';
				tr+='</tr>';
				$('#tservice tbody').append(tr);
				$('#total_service').html($('#tservice tbody tr').length);
			});
			break
		case 'product_vases':
			...
			saveFBVas(vas=>{
				console.log('VAS',vas);
				tr='<tr thisid='+vas.lastid+'>';
				tr+='<td class="servicecategory">VAS</td>';
				tr+='<td class="info">';
				tr+='<a>'+vas.vas+'</a>';
				tr+='</td>';
				tr+='<td>';
				...
				tr+='</button>';
				tr+='<ul class="dropdown-menu pull-right">';
				tr+='<li class="remove_vas"><a>Hapus</a></li>';
				tr+='</ul>';
				tr+='</div>';
				tr+='	</td>';
				tr+='</tr>';
...
$('#tservice tbody').on('click','.remove_device',function(){
	that = $(this);
	$('#tservice tbody tr').removeClass('selected');
	tr = that.stairUp({level:4});
	tr.addClass('selected');
...
$('#tservice tbody').on('click','.remove_vas',function(){
	that = $(this);
	$('#tservice tbody tr').removeClass('selected');
	tr = that.stairUp({level:4});
	tr.addClass('selected');
